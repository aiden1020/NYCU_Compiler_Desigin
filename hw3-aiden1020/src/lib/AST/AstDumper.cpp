...
void AstDumper::visit(ConstantValueNode &p_constant_value)
{
    ...
    switch (p_constant_value.getType())
    {
    case ConstantValueNode::Type::Integer:
        std::printf("%d\n", p_constant_value.getIntValue());
        break;
    case ConstantValueNode::Type::Real:
        std::printf("%f\n", p_constant_value.getRealValue());
        break;
    case ConstantValueNode::Type::Boolean:
        std::printf("%s\n", p_constant_value.getBoolValue() ? "true" : "false");
        break;
    case ConstantValueNode::Type::String:
...
{
    printIndent();
    std::string prototype_str = p_function.getReturnTypeCString();
    prototype_str += " (";

    bool first = true;
    for (const auto &decl : p_function.getDeclarations())
    {
        for (const auto &id : decl->getIdentifiers())
        {
            if (first)
            {
                first = false;
            ...
            prototype_str += id->getTypeCString();
        }
    }
    prototype_str += ")";
    ...
    std::printf("function declaration <line: %u, col: %u> %s %s\n",
                ...
                p_function.getNameCString(), prototype_str.c_str());
